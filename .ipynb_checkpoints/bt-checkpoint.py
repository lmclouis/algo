...
model_params=[
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 250, 10), 'l_threshold':np.arange(0, 2.5, 0.1)},
        {'resolution':'1h', 'l_window':np.arange(100, 3000, 100), 'l_threshold':np.arange(0, 2.5, 0.1)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 200, 10), 'l_threshold':np.arange(0, 0.8, 0.02)},
        {'resolution':'1h', 'l_window':np.arange(100, 3000, 100), 'l_threshold':np.arange(0, 0.8, 0.02)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 20, 1), 'l_threshold':np.arange(0, 30, 1)},
        {'resolution':'1h', 'l_window':np.arange(0, 20, 1), 'l_threshold':np.arange(0, 30, 1)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 20, 1), 'l_threshold':np.arange(0, 0.3, 0.02)},
        {'resolution':'1h', 'l_window':np.arange(0, 20*24, 1*24), 'l_threshold':np.arange(0, 0.3, 0.02)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(10, 110, 10), 'l_threshold':np.arange(0, 0.5, 0.05)},
        {'resolution':'1h', 'l_window':np.arange(10, 110, 10), 'l_threshold':np.arange(0, 0.5, 0.05)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 200, 10), 'l_threshold':np.arange(0, 200, 10)},
        {'resolution':'1h', 'l_window':np.arange(100, 3000, 100), 'l_threshold':np.arange(100, 3000, 100)},
...
},
{
    ...
    [{'resolution':'24h', 'l_window':np.arange(0, 200, 10), 'l_threshold':np.arange(0, 2.0, 0.02)},
        {'resolution':'1h', 'l_window':np.arange(100, 3000, 100), 'l_threshold':np.arange(0, 2.0, 0.02)},
...
def get_price_data(since, exchange : str ='bybit', resolution: str = '24h', asset_output = 'BTC'):
    ...
    if resolution == '10m':
        df = fetch_ohlcv(f"{asset_output}/USDT:USDT", '5m', int(since.timestamp()*1000)) #<<<
        # df = df.drop(['o', 'h', 'l', 'v'], axis=1)
        df['t'] = pd.to_datetime(df['t'], unit='ms')
        df = df.set_index('t').resample('10T').agg({'c': 'last'}).reset_index()
        df['t'] = df['t'].astype(int) // 10**9 
    elif resolution == '24h':
        df = fetch_ohlcv(f"{asset_output}/USDT:USDT", '1d', int(since.timestamp()*1000)) #<<<
        # df = df.drop(['o', 'h', 'l', 'v'], axis=1)
        df['t'] = df['t'] // 1000
        # df.columns=['t','v']
    elif resolution == '1h':
        df = fetch_ohlcv(f"{asset_output}/USDT:USDT", '1h', int(since.timestamp()*1000)) #<<<
        # df = df.drop(['o', 'h', 'l', 'v'], axis=1)
        df['t'] = df['t'] // 1000
    else:
        sys.exit()
...
def get_df_endpoints(asset_input: str = 'BTC', resolution = '24h'):
    ...
    res = requests.get(f"{URL_BASE}/v2/metrics/endpoints", params={"api_key": api.api_glassnode['api_key']})
    df = pd.read_json(res.text)
    df = df[df['assets'].apply(lambda x: any(y['symbol'] == asset_input for y in x))]
...
def backtesting(df_o, model, window, threshold, long_or_short, direction):
    ...
    def z_score(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                ...
                df['pos'] = np.where(df['z'] > threshold, 1, np.where(df['z'] < -threshold, 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where(df['z'] > threshold, 0, np.where(df['z'] < -threshold, -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where(df['z'] > threshold, -1, np.where(df['z'] < -threshold, 1, 0))
            elif long_or_short == 'long':
    ...
    def ma_diff(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where(df['ma_diff'] > threshold, 1, np.where(df['ma_diff'] < -threshold, -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where(df['ma_diff'] > threshold, 1, np.where(df['ma_diff'] < -threshold, 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where(df['ma_diff'] > threshold, 0, np.where(df['ma_diff'] < -threshold, -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where(df['ma_diff'] > threshold, -1, np.where(df['ma_diff'] < -threshold, 1, 0))
            elif long_or_short == 'long':
...
# models = ['z_score', 'ma_diff', 'rsi', 'ma_cross', 'iqr', 'min_max', 'percentile'][0:1]
    ...
    def rsi(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where((df['rsi'] > (100 - threshold)), 1, np.where(df['rsi'] < threshold, -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where((df['rsi'] > (100 - threshold)), 1, np.where(df['rsi'] < threshold, 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where((df['rsi'] > (100 - threshold)), 0, np.where(df['rsi'] < threshold, -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where((df['rsi'] > (100 - threshold)), -1, np.where(df['rsi'] < threshold, 1, 0))
            elif long_or_short == 'long':
    ...
    def ma_cross(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where((df['pre'] < 0) & (df['post'] > 0), 1, np.where((df['pre'] > 0) & (df['post'] < 0), -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where((df['pre'] < 0) & (df['post'] > 0), 1, np.where((df['pre'] > 0) & (df['post'] < 0), 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where((df['pre'] < 0) & (df['post'] > 0), 0, np.where((df['pre'] > 0) & (df['post'] < 0), -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where((df['pre'] < 0) & (df['post'] > 0), -1, np.where((df['pre'] > 0) & (df['post'] < 0), 1, 0))
            elif long_or_short == 'long':
    ...
    def iqr(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where(df['factor'] > df['upper_bound'], 1, np.where(df['factor'] < df['lower_bound'], -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where(df['factor'] > df['upper_bound'], 1, np.where(df['factor'] < df['lower_bound'], 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where(df['factor'] > df['upper_bound'], 0, np.where(df['factor'] < df['lower_bound'], -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where( df['factor'] > df['upper_bound'], -1, np.where(df['factor'] < df['lower_bound'], 1, 0))
            elif long_or_short == 'long':
    ...
    def min_max(df, window, threshold, long_or_short, direction):
        ...
        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where(df['x'] > (1 - threshold), 1, np.where(df['x'] < (threshold), -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where(df['x'] > (1 - threshold), 1, np.where(df['x'] < (threshold), 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where(df['x'] > ( 1 - threshold), 0, np.where(df['x'] < (threshold), -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where(df['x'] > (1 - threshold), -1, np.where(df['x'] < (threshold), 1, 0))
            elif long_or_short == 'long':
    ...
    def percentile(df, window, threshold, long_or_short, direction):
        ...
        df['percentile_high'] = df['factor'].rolling(window).quantile(1 - threshold)
        df['percentile_low'] = df['factor'].rolling(window).quantile(threshold)

        if direction == 'momentum':
            if long_or_short == 'long short':
                df['pos'] = np.where(df['factor'] > df['percentile_high'], 1, np.where(df['factor'] < df['percentile_low'], -1, 0))
            elif long_or_short == 'long':
                df['pos'] = np.where(df['factor'] > df['percentile_high'], 1, np.where(df['factor'] < df['percentile_low'], 0, 0))
            elif long_or_short == 'short':
                df['pos'] = np.where(df['factor'] > df['percentile_high'], 0, np.where(df['factor'] < df['percentile_low'], -1, 0))
        else:
            if long_or_short == 'long short':
                df['pos'] = np.where(df['factor'] > df['percentile_high'], -1, np.where(df['factor'] < df['percentile_low'], 1, 0))
            elif long_or_short == 'long':
